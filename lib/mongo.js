...
class MongoDB extends EventEmitter {
  ...
  connect() {
    return MongoClient.connect(this.url)
      ...
      .catch(error => /* istanbul ignore next */ this.emit('error', error));
  }
  ...
  deleteMany(name, args = {}) {
    // default filter is undefined to avoid unexpected delete
  ...
  createIndex(name, args = {}) {
    ...
    const options = args.options || {};
    this.db.collection(name).createIndex(fieldOrSpec, options);
  }
